...
const Header = () => {
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    if (href === '#top') {
      ...
      if (element) {
        const headerHeight = 100; // header高度
      ...
      <div className="header-container">
        ...
        <div className="header-left">
          <div className="header-brand-container">
            ...
            />
            <h1 className="header-brand">Solidus</h1>
          </div>
          <nav className="header-nav">
            {navItems.map((item, index) => (
              <Link key={index} href={item.href} className="nav-link" onClick={(e) => handleNavClick(e, item.href)}>
                {item.label}
              </Link>
            ))}
          </nav>
        ...
        <div className="header-right">
          <div className="social-icons">
            <Link href="https://t.me/Solidus_Official" target="_blank" className="social-icon">
              <svg width="20" height="20" viewBox="0 0 50 50" fill="none">
                <path d="M46.137,6.552c-0.75-0.636-1.928-0.727-3.146-0.238l-0.002,0C41.708,6.828,6.728,21.832,5.304,22.445	c-0.259,0.09-2.521,0.934-2.288,2.814c0.208,1.695,2.026,2.397,2.248,2.478l8.893,3.045c0.59,1.964,2.765,9.21,3.246,10.758	c0.3,0.965,0.789,2.233,1.646,2.494c0.752,0.29,1.5,0.025,1.984-0.355l5.437-5.043l8.777,6.845l0.209,0.125	c0.596,0.264,1.167,0.396,1.712,0.396c0.421,0,0.825-0.079,1.211-0.237c1.315-0.54,1.841-1.793,1.896-1.935l6.556-34.077	C47.231,7.933,46.675,7.007,46.137,6.552z M22,32l-3,8l-3-10l23-17L22,32z" fill="currentColor"/>
              </svg>
            </Link>
            
            <Link href="https://x.com/Solidus_Token" target="_blank" className="social-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" fill="currentColor"/>
              </svg>
            </Link>
            
            <Link href="https://dexscreener.com/solana/EG4iar96FkhfnAa7UQbkBARwDCU8hyncd2igEqifQhqA" target="_blank" className="social-icon">
              <svg width="20" height="20" viewBox="0 -.058 754.779 867.058" fill="none">
            ...
            <Link href="https://www.dextools.io/app/en/solana/pair-explorer/EG4iar96FkhfnAa7UQbkBARwDCU8hyncd2igEqifQhqA?t=1749182596123" target="_blank" className="social-icon">
              <svg width="20" height="20" viewBox="0 0 252 300" fill="none" fillRule="evenodd">
                <path d="M151.818 106.866c9.177-4.576 20.854-11.312 32.545-20.541 2.465 5.119 2.735 9.586 1.465 13.193-.9 2.542-2.596 4.753-4.826 6.512-2.415 1.901-5.431 3.285-8.765 4.033-6.326 1.425-13.712.593-20.419-3.197m1.591 46.886l12.148 7.017c-24.804 13.902-31.547 39.716-39.557 64.859-8.009-25.143-14.753-50.957-39.556-64.859l12.148-7.017a5.95 5.95 0 003.84-5.845c-1.113-23.547 5.245-33.96 13.821-40.498 3.076-2.342 6.434-3.518 9.747-3.518s6.671 1.176 9.748 3.518c8.576 6.538 14.934 16.951 13.821 40.498a5.95 5.95 0 003.84 5.845zM126 0c14.042.377 28.119 3.103 40.336 8.406 8.46 3.677 16.354 8.534 23.502 14.342 3.228 2.622 5.886 5.155 8.814 8.071 7.897.273 19.438-8.5 24.796-16.709-9.221 30.23-51.299 65.929-80.43 79.589-.012-.005-.02-.012-.029-.018-5.228-3.992-11.108-5.988-16.989-5.988s-11.76 1.996-16.988 5.988c-.009.005-.017.014-.029.018-29.132-13.66-71.209-49.359-80.43-79.589 5.357 8.209 16.898 16.982 24.795 16.709 2.929-2.915 5.587-5.449 8.814-8.071C69.31 16.94 77.204 12.083 85.664 8.406 97.882 3.103 111.959.377 126 0m-25.818 106.866c-9.176-4.576-20.854-11.312-32.544-20.541-2.465 5.119-2.735 9.586-1.466 13.193.901 2.542 2.597 4.753 4.826 6.512 2.416 1.901 5.432 3.285 8.766 4.033 6.326 1.425 13.711.593 20.418-3.197" fill="currentColor"/>
